...
class UserView {
	...
	public static function showDetails() {
		...
		$base = (array_key_exists('base', $_SESSION))?$_SESSION['base']:"";
		$userThreads = (array_key_exists('userThreads', $_SESSION))?
		$_SESSION['userThreads']:array();
		if (is_null($user))
			...
			echo '<section><h2>My threads</h2>';
			echo '<ul>';
			foreach ($userThreads as $thread) {
				echo '<li> <a href = "/'.$base.'/thread/show/'.
						$thread->getThreadId().'">Thread '.
						$thread->getThreadId().'</a></li>';
			}
			echo '</ul></section>';
			echo '</div>';
		}
